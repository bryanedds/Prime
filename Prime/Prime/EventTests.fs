...
module EventTests =
    ...
    type [<ReferenceEquality>] TestWorld =
        ...
        interface EventSystem<TestWorld> with
            ...
            member this.GetGlobalParticipantSpecialized () = EventSystemDelegate.getGlobalParticipantSpecialized this.TestEventSystemDelegate
            member this.GetGlobalParticipantGeneralized () = EventSystemDelegate.getGlobalParticipantGeneralized this.TestEventSystemDelegate
            ...
            member this.PublishEventHook (participant : Participant) publisher eventData eventAddress eventTrace subscription world =
                ...
                | :? GlobalParticipantGeneralized -> EventSystem.publishEvent<'a, 'p, Participant, TestWorld> participant publisher eventData eventAddress eventTrace subscription world
                | :? TestParticipant -> EventSystem.publishEvent<'a, 'p, TestParticipant, TestWorld> participant publisher eventData eventAddress eventTrace subscription world
                | _ -> failwithumf ()
        ...
        static member make eventTracer eventTracing eventFilter globalParticipant globalContext =
            { TestState = 0; TestEventSystemDelegate = EventSystemDelegate.make eventTracer eventTracing eventFilter globalParticipant globalContext }
    ...
    let [<Fact>] subscribeWorks () =
        ...
        let world = EventSystem.subscribe incTestState TestEvent TestParticipantSpecialized world
    ...
    let [<Fact>] subscribeWithResolutionWorks () =
        let world = TestWorld.make ignore false EventFilter.Empty TestParticipantSpecialized TestParticipantGeneralized
        let world = EventSystem.subscribePlus (makeGuid ()) incTestStateAndResolve TestEvent TestParticipantSpecialized world |> snd
        let world = EventSystem.subscribePlus (makeGuid ()) incTestStateAndCascade TestEvent TestParticipantSpecialized world |> snd
        let world = EventSystem.publish 0 TestEvent EventTrace.empty TestParticipantSpecialized world
        Assert.Equal (1, world.TestState)
    ...
    let [<Fact>] unsubscribeWorks () =
        let key = makeGuid ()
        let world = TestWorld.make ignore false EventFilter.Empty TestParticipantSpecialized TestParticipantGeneralized
        let world = EventSystem.subscribePlus key incTestStateAndResolve TestEvent TestParticipantSpecialized world |> snd
    ...
    let [<Fact>] sumWorks () =
        ...
        let world = EventSystem.publish 1 TestEvent EventTrace.empty TestParticipantSpecialized world
        Assert.Equal (1, world.TestState)
        let world = EventSystem.publish true TestEvent2 EventTrace.empty TestParticipantSpecialized world
    ...
    let [<Fact>] chainWorks () =
        ...
        // and so on...
        let world = EventSystem.publish 3 TestEvent EventTrace.empty TestParticipantSpecialized world
        Assert.Equal (3, world.TestState)
