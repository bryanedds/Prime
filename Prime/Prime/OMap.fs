...
module OMap =
    ...
    let isEmpty map =
        FStack.isEmpty map.Values

    /// Check that an OMap is empty.
    let notEmpty map =
        FStack.notEmpty map.Values

    /// Add a value with the key to an OMap.
    let add (key : 'k) (value : 'v) map =
        { Indices = UMap.add key (FStack.length map.Values) map.Indices
          Values = FStack.conj (key, value) map.Values }

    /// Remove a value with the given key from an OMap.
    let remove (key : 'k) map =
        match UMap.tryFind key map.Indices with
        | Some index -> { Indices = UMap.remove key map.Indices; Values = FStack.removeAt index map.Values }
        | None -> map
