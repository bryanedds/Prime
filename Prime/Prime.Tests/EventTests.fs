    ...
    type [<ReferenceEquality>] TestWorld =
        ...
        interface EventSystem<TestWorld> with
            ...
            member this.PublishEventHook (simulant : Simulant) publisher eventData eventAddress eventTrace subscription world =
                match simulant with
                | :? GlobalSimulantGeneralized -> EventSystem.publishEvent<'a, 'p, Simulant, TestWorld> simulant publisher eventData eventAddress eventTrace subscription world
                | :? TestSimulant -> EventSystem.publishEvent<'a, 'p, TestSimulant, TestWorld> simulant publisher eventData eventAddress eventTrace subscription world
                | _ -> failwithumf ()
        static member incTestState this =
            { this with TestState = inc this.TestState }
        static member make eventTracer eventTracing eventFilter globalSimulant globalContext =
            { TestState = 0; TestEventSystemDelegate = EventSystemDelegate.make eventTracer eventTracing eventFilter globalSimulant globalContext }
