...
-   use PascalCasing for privatized fields but with a trailing underscore, such as `MyField_`. Prime's symbolic serializer with detect the trailing underscore when the SymbolicExpansion attribute is used to prettify the name by stripping the trailing underscore during serialization.
